...
- **4e round — endpoint agrégé** : même à 4 requêtes, chacune paie le plancher de latence réseau hôte (mesuré ~300-400ms même pour une réponse triviale). Ajout de `GET /api/sessions/{id}/state` (`SessionStateController`, contrôleur Symfony classique, pas une opération API Platform — évite de redéfinir un DTO de sortie juste pour agréger) qui renvoie `{ session, themes, questions, players }` en sérialisant chaque partie avec le serializer `jsonld` déjà utilisé par API Platform (même format que les endpoints individuels, donc le front n'a presque rien à changer). `GameView.vue` et `SessionEditView.vue` l'utilisent pour leur chargement initial (4 requêtes → 1). Mesuré : session à 9 joueurs (10 thèmes, 54 questions, 9 joueurs) chargée en ~310-365ms au lieu de ~2,8s.

## Mode facile (post-plan initial)

- **Objectif** : un joueur peut être déclaré en "mode facile" — pendant son tour, les questions de son propre thème gardent leur couleur affichée sur la grille (au lieu de passer au noir comme le reste), pour l'aider à repérer plus vite son thème.
- **Backend** : nouveau champ `Player::$easyMode` (bool, `easyMode`/`isEasyMode()`/`setEasyMode()` — pas de piège JSON-LD ici puisque le nom de propriété ne contient pas déjà "is", contrairement à l'ancien bug `isShuffled`/`isAnswered`), migration avec `DEFAULT false` (des joueurs existaient déjà en base). Champ normalement writable (à la différence de `score`), modifiable via `PATCH /api/players/{id}`.
...
## Thème hardcore — punition du vol de question "mode facile" (post-plan initial)

**Règle** (précisée par l'utilisateur) : si un joueur *sans* mode facile pioche une question d'un thème *en* mode facile (un "vol"), la question est remplacée par une question piochée dans une réserve "hardcore" (jamais placée dans la grille au départ). Le score suit la règle existante (2 points, thème différent du sien). Pour que le joueur en mode facile ait quand même assez de questions de son propre thème à répondre malgré le vol, son thème reçoit 1 question "tampon" en plus (Q+1 au lieu de Q) — financée en retirant 1 question au thème bonus.

- **Invariant vérifié** : total questions "en jeu" (thèmes normaux + bonus, hardcore exclu car hors grille) = `normalThemes × Q + (nb thèmes normaux) - (nb thèmes mode facile) + (nb thèmes mode facile) × 1` = `normalThemes × (Q+1)` — inchangé par rapport à avant l'ajout du mode facile/hardcore, donc "chacun répond à Q+1 questions" reste vrai globalement.
- **Backend** :
  - `Theme::$hardcore` (bool, `hardcore`/`isHardcore()`/`setHardcore()`), migration avec `DEFAULT false`.
  - `ResetSessionProcessor` : les questions du thème hardcore sont explicitement remises à `number = null` (jamais dans le tirage `range(1, ...)` de la grille) ; seules les questions des thèmes normaux/bonus reçoivent un numéro.
  - Nouvelle opération `POST /api/questions/{id}/select` (`SelectQuestionProcessor`, `deserialize: false` — pas de corps de requête nécessaire, même pattern que `/reset`) : appelée quand un joueur clique un chiffre (remplace l'ancien PATCH direct `answered:true` fait par le front). Logique : si le joueur courant n'est *pas* en mode facile ET que le propriétaire du thème de la question cliquée *est* en mode facile → cherche une question hardcore non utilisée (`number IS NULL`, `answered = false`) de la session, lui donne le numéro de la question cliquée, remet le numéro de la question cliquée à `null` (retour en réserve, injouable pour le reste de la partie), renvoie la question hardcore comme "question active". Sinon (cas normal), renvoie simplement la question cliquée avec `answered = true`. Si la réserve hardcore est épuisée, dégrade gracieusement (pas d'échange, pas d'erreur).
  - Aucun changement nécessaire dans `ResolveQuestionProcessor` : le thème hardcore n'appartenant à aucun joueur, la comparaison `currentPlayer.theme === question.theme` y est toujours fausse → 2 points automatiquement, exactement la règle demandée, sans code spécifique.
- **Édition de session** (`SessionEditView.vue`) :
  - `hardcoreThemes` (filtre `t.hardcore`), exclu de `normalThemes` (qui excluait déjà `bonus`).
  - `ensureHardcoreTheme()` (même pattern que `ensureBonusTheme()`) : crée le thème "Hardcore" (couleur fixe `#f43f5e`, hors palette des 10 couleurs, jamais proposée aux thèmes normaux) avec `Q` questions vides, `Q` étant `baseQuestionCount` (le minimum des comptes de questions "de base" des thèmes normaux, buffer mode-facile déduit — pour ne pas se caler par erreur sur un thème qui a déjà son +1).
  - `toggleEasyMode()` étendu : à l'activation, ajoute 1 question au thème (buffer) et en retire 1 au bonus (`removeOneQuestion`, préfère une question non répondue) ; à la désactivation, l'inverse. `addQuestionRound()` (bouton "manche") ajoute aussi 1 question au thème hardcore (Q augmente pour tout le monde). `readiness` mis à jour : compare les comptes des thèmes normaux **après déduction du buffer mode-facile**, exige exactement 1 thème hardcore avec `Q` questions, et le bonus avec `nb thèmes normaux − nb thèmes mode facile` questions.
  - Libellé "— 💀 Hardcore" à côté du thème dans les sections "Thèmes & joueurs" et "Questions" (même traitement que "— ⭐ Bonus").
- **Écran de jeu** (`GameView.vue`) :
  - `selectQuestion()` appelle désormais `POST /questions/{id}/select` au lieu du `PATCH answered:true` direct. Si la question renvoyée diffère de celle cliquée (échange survenu), la question cliquée repasse à `number: null` en local et la question hardcore (déjà présente dans `questions.value`, chargée avec les autres) reçoit le numéro — pas besoin de la rappatrier depuis le serveur.
  - `allAnswered` recalculé à partir de `gridOrder` (les numéros réellement sur la grille) plutôt que de `questions.value` en entier : les questions hardcore inutilisées et les questions "volées" renvoyées en réserve ont `number: null` et bloqueraient sinon la détection de fin de partie / l'apparition du podium.
  - Le thème hardcore reste invisible dans la sidebar (`sidebarEntries`) et la légende du compte à rebours (`legendThemes`) — c'est une réserve surprise, pas une catégorie à annoncer à l'avance.
- **Jeu de test** : thème "Hardcore" (5 questions) + 1 question tampon sur "Séries TV" (déjà en mode facile, activé par l'utilisateur en testant la fonctionnalité précédente) + 1 question bonus retirée, ajoutés à la session 9 joueurs. Testé via l'API (pas de navigateur disponible) : reset → questions hardcore bien à `number: null` ; un joueur non-facile qui pioche une question "Séries TV" reçoit bien une question hardcore avec le même numéro, l'originale repasse en réserve ; un pick sur un thème normal ne déclenche aucun échange.

**Révision — mécanique abandonnée, remplacée par un verrouillage simple** : le vol/échange contre une question hardcore s'est révélé trop compliqué pour le bénéfice apporté. Nouvelle règle : les questions d'un thème en mode facile sont simplement **grisées et non cliquables** pour tout le monde sauf leur propriétaire, en dehors de son tour — elles ne peuvent donc plus jamais être "volées", pas besoin de remplacement.
- `SelectQuestionProcessor` (endpoint `POST /questions/{id}/select`, toujours utilisé côté front) ne fait plus qu'un `answered = true` — toute la logique d'échange a été retirée.
- Front (`GameView.vue`) : `isLockedForOthers(question)` — vrai si le thème de la question a un joueur propriétaire en mode facile différent du joueur courant — alimente le `:disabled` du `GridNumber` (en plus de `answered`/`countdown`). `GridNumber.vue` a maintenant une prop `answered` distincte de `disabled`, pour qu'une case verrouillée (mais pas encore répondue) affiche toujours son chiffre au lieu d'une coche ✓ (réservée aux questions réellement répondues).
- `SessionEditView.vue` : plus de création automatique de thème "Hardcore" (`ensureHardcoreTheme` retiré), plus de tampon de question sur les thèmes en mode facile (`toggleEasyMode` ne touche plus qu'au champ `easyMode` du joueur), plus d'exigence d'une catégorie hardcore dans `readiness`. Le champ `Theme::hardcore` et son filtrage (`!t.hardcore` dans `normalThemes`, libellé "💀 Hardcore") sont **conservés** côté front/back uniquement pour ne pas casser l'affichage/la lecture des sessions existantes qui ont déjà un thème Hardcore en base (ses questions restent à `number: null` pour toujours via `ResetSessionProcessor`, donc invisibles en jeu) — mais plus aucune nouvelle session n'en crée.
- ⚠️ **Impact sur les données existantes** : la session "Test 9 joueurs (grille XXL)" a un tampon historique (thème "Séries TV" à 6 questions au lieu de 5, catégorie "Bonus" à 8 au lieu de 9) financé par l'ancienne mécanique. Le calcul de `readiness` ne compense plus cet écart (il n'y a plus de notion de "tampon"), donc cette session précise affichera "pas prêt" tant que ce déséquilibre n'est pas corrigé à la main (ajouter 1 question à "Séries TV" ou en retirer 1 à "Bonus", ou l'inverse — au choix, via les boutons déjà existants de l'écran d'édition).
...
- **Révision "rééquilibrage"** (moins de bleu-vert, un vrai jaune) : seulement 2 teintes froides (teal + bleu, au lieu de 3), jaune ajouté en poussant volontairement sa luminosité au-dessus de la bande validée (sinon un jaune saturé se lit comme olive/moutarde). Palette de cette étape : `#da1a69 #e84415 #da7e1a #e2b936 #569f18 #03816b #318eb5 #8050eb #b31ce4 #e51eb9`.
- **Révision "plus saturé"** (retour utilisateur : certaines couleurs se distinguaient mal) : teal (`#03816b`) et bleu (`#318eb5`) étaient déjà à leur chroma max *pour leur luminosité de l'époque*, mais cette luminosité tombait dans une zone où le gamut sRGB est étroit pour ces teintes — elles lisaient grisâtres à côté des autres. Remonter leur L à ~0.665 (dans la bande validée) puis re-maximiser la chroma à ce nouveau point y débloque nettement plus de saturation (+19 % teal, +23 % bleu). Même traitement pour le violet (`#8050eb`, +18 % de chroma à L inchangée). L'orange (`#da7e1a`, L=0.68) est aussi ramené à L=0.665 pour rentrer dans la bande (gain de chroma négligeable, mais élimine le FAIL de bande). Rouge/rose/magenta/vert/jaune laissés inchangés : déjà à leur chroma max pour leur luminosité, aucune marge disponible sans re-choisir aussi leur L (ce qui rapprocherait certaines paires adjacentes — testé, pire pour orange/rouge-orangé). Palette de cette étape : `#da1a69 #e84415 #d47a1a #e2b936 #569f18 #1fab90 #1ea1d2 #8246f8 #b31ce4 #e51eb9`.
- **Révision "rose + gris"** (retour utilisateur : remplacer un orange et un violet par un rose et un gris) : l'orange (`#d47a1a`) devient un rose doux `#f96caa` (L=0.72, H=355 — délibérément au-dessus de la bande validée, même logique que le jaune : un rose foncé et saturé s'écraserait vers les teintes crimson/magenta déjà présentes au lieu de se lire comme une couleur à part) ; le violet (`#8246f8`) devient un gris neutre `#6b727a` (C=0.015, délibérément sous le plancher de chroma — c'est le but recherché ; l'identification reste portée par le nom du thème toujours affiché à côté de la pastille, jamais par la couleur seule). Palette finale : `#da1a69 #e84415 #f96caa #e2b936 #569f18 #1fab90 #1ea1d2 #6b727a #b31ce4 #e51eb9`. Le plancher séparation daltonisme/vision-normale sur les paires adjacentes les plus proches reste sous le seuil idéal (limite déjà documentée ci-dessus, structurelle au-delà de ~3 couleurs simultanées pour un rendu où deux teintes quelconques peuvent se retrouver côte à côte, comme la grille de ce jeu) — la mitigation (nom du thème toujours affiché) reste donc nécessaire.
